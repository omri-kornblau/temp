...
class trajectory_point(object):
    ...
    def update_velocities_forward (self, prev_point, max_vel):        
        new_vel = sign(self.left_dist)*(abs(2*prev_point.left_acc*self.left_dist + prev_point.left_vel**2))**0.5
        self.left_vel = min(max_vel, new_vel, key=abs)

        new_vel = sign(self.right_dist)*(abs(2*prev_point.right_acc*self.right_dist + prev_point.right_vel**2))**0.5
        self.right_vel = min(max_vel, new_vel, key=abs)
        
    def update_velocities_backward (self, prev_point, max_vel):                
        new_vel = sign(prev_point.left_dist)*(abs(2*prev_point.left_acc*prev_point.left_dist + prev_point.left_vel**2))**0.5
        self.left_vel = min(max_vel, self.left_vel, new_vel, key=abs)
        
        new_vel = sign(prev_point.right_dist)*(abs(2*prev_point.right_acc*prev_point.right_dist + prev_point.right_vel**2))**0.5
    ...
    def update_point(self, prev_point, dt_left, dt_right, faster_side, max_acc, max_vel):
        ...
        if faster_side == "right":
            self.left_vel  = 2*self.left_dist/dt_right - prev_point.left_vel
            ...
            self.right_vel  = 2*self.right_dist/dt_left - prev_point.right_vel#self.left_vel*time_ratio*abs(self.left_dist/self.right_dist)
            self.right_acc  = max(max_acc-max_acc*(abs(self.right_vel)/max_vel), 0.1) #self.left_acc*time_ratio*(self.left_vel-prev_point.left_vel)/(self.right_vel-prev_point.right_vel)
            self.left_acc   = max(max_acc-max_acc*(abs(self.left_vel)/max_vel), 0.1)
    ...
    def update_point_backward(self, prev_point, dt_left, dt_right, faster_side, max_acc, max_vel):
        ...
        if faster_side == "right":
            self.left_vel  = 2*prev_point.left_dist/dt_right - prev_point.left_vel
            self.right_acc = max(max_acc-max_acc*(abs(self.right_vel)/max_vel), 0.1)
            self.left_acc  = max(max_acc-max_acc*(abs(self.left_vel)/max_vel), 0.1)
            self.time = prev_point.time-dt_right

        elif faster_side == "left":
            self.right_vel  = 2*prev_point.right_dist/dt_left - prev_point.right_vel#self.left_vel*time_ratio*abs(self.left_dist/self.right_dist)
            self.right_acc  = max(max_acc-max_acc*(abs(self.right_vel)/max_vel), 0.1) #self.left_acc*time_ratio*(self.left_vel-prev_point.left_vel)/(self.right_vel-prev_point.right_vel)
            self.left_acc   = max(max_acc-max_acc*(abs(self.left_vel)/max_vel), 0.1)
            self.time = prev_point.time-dt_left
